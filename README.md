...
### v1.2.2 (2024-01-10)
**多端同步优化 & 用户体验改进**
- 🔧 修复多端同步时的配置读取错误（兼容不同版本酒馆后端）
- 🛡️ 实现健壮的数据迁移策略，保护老用户配置不丢失
- 🔇 优化云端同步体验：改为静默后台操作，不再显示打断性的成功提示
- ✅ 修复强制同步按钮的 403 错误（添加 CSRF Token）
- 📦 添加智能配置解析函数，自动适配字符串包裹的 JSON 格式

### v1.2.1 (2024-01-09)
**紧急修复多端同步 API 逻辑**
- 修复强制同步按钮从服务器读取配置失败的问题
- 添加防缓存机制和详细调试日志

### v1.2.0 (2024-01-08)
**多重更新：表格自定义、分批总结、云端同步优化**
- 完全移除虚构的 API 端点
- 改用 SillyTavern 原生存储系统
- 优化配置同步机制
